<template>
  <div class="container-fluid">
    ...
    <div class="row">
      ...
      <router-link :to="{ name: 'DestinationDetail', params: {id: destination.id }  }" type="button" class="col-4 text-center carte rounded-pill shadow-lg nav-link pr-3" v-for="destination in destinations" v-bind:key="destination.id">
          <h2> {{ destination.dest_name }} </h2>
          </router-link>
...
export default {
  ...
  methods: {
    destinationList () {
      axios.get('http://localhost:8000/api/destinations').then(response => this.destinations = response.data).catch(error => console.log(error))
